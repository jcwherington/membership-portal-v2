...
class Membership:
    ...
    def serialize(cls, tuple):
        property_names = [humps.camelize(field.name.lstrip('_')) for field in fields(cls)]
        property_names.insert(0, "id")
        member_dict = { key: value.isoformat() if isinstance(value, date) else value for key, value in zip(property_names, tuple) }
